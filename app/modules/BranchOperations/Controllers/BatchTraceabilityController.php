...
class BatchTraceabilityController extends Controller
{
    ...
    private function consumption(\Illuminate\Support\Collection $supplyItemIds): array
    {
        ...
        return compact('remaining', 'transferred', 'wasted', 'sold', 'adjusted');
    }
    ...
    private function mapBatch(SupplyItem $item, array $c): array
    {
        return [
            ...
            'sold_quantity' => round((float) ($c['sold'][$item->id] ?? 0), 3),
    ...
    public function export(Request $request)
    {
        ...
        $columns = ['رقم الدفعة', 'فاتورة الشراء', 'المورّد', 'المنتج', 'الكود', 'تاريخ الشراء', 'الكمية الأصلية', 'المتبقي', 'منقول', 'هالك', 'مباع'];
        $tableRows = $rows->map(fn ($r) => [
            ...
            $r['wasted_quantity'], $r['sold_quantity'],
        ])->all();
